...
# W1.6 新增: cube-agent-server 地址 (C7 cron 用)
FRESHCHECK_C7_CUBE_URL=http://127.0.0.1:8088
...
# 期望 log (按顺序):
#   "freshcheck: 13 张表就绪"
#   "freshcheck C7 cron 启动 (branch=0001 cube_url=...)"
```
...
# 5) 手动触发 C7
Invoke-RestMethod -Method POST -Headers @{Authorization="Bearer $token"} 'http://127.0.0.1:8089/api/v1/freshcheck/admin/sync-check' -Body '{}' -ContentType 'application/json'
# 期望: SyncResult JSON (cube_value / source_value / diff_pct / ok 字段)
...
### 5.1 C7 cron 告警
- 每小时 7-21 点跑一次
- 差异 > 0.01% → 写 `freshcheck_alert` (severity 取决于连续失败次数)
- 告警查询:
  ```sql
  SELECT * FROM freshcheck_alert
  WHERE rule_code = 'C7' AND status = 'open'
...
### 5.3 cube-agent-server 不可用
- C7 cron 调 SourceClient.Aggregate 超时/失败 → log "freshcheck C7 校验失败"
- 连续 3 次 → severity='block', 后续 W3 周期结算会被 C7 阻断
...
| 12 行阈值 seed 缺失 | 老库没 ON CONFLICT | 手动跑: `INSERT INTO freshcheck_threshold ...` (见 [data-model.md §4](freshcheck-data-model.md#4-freshcheck_threshold--配置-业务阈值-k-v)) |
| 5 perm seed 缺失 | 同上 | 手动跑 role_permissions seed |
| C7 cron "cube-agent-server datasources endpoint status 503" | cube-agent-server 没起或 hbpos 没配 | 检查 cube-agent-server .env 和 config/datasources.yaml |
| C7 cron "source aggregate: timeout" | SQL Server 慢 / 索引缺失 | 检查 t_rm_saleflow.oper_date 索引, 加 `CREATE INDEX ... ON t_rm_saleflow(oper_date)` |
...
- W1 已知限制:
  - [ ] `inventory_current` cube 的 `avg_cost` measure 是简单 AVG, 不等于加权平均. W1.7 / W3 改 cube 加 `weighted_avg_cost`
  - [ ] cube-agent-server `/admin/source-direct` 暂不支持 GROUP BY
  - [ ] C7 cron 只校验 24h 销售总额, 采购/库存差异校验待 W3 加
